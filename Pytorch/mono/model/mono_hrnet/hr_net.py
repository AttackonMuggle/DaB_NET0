...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        list18 = torch.cat(list18, dim=1)
        list36 = torch.cat(list36, dim=1)
        list72 = torch.cat(list72, dim=1)
        features = torch.cat(features, dim=1)
        
        #mixed_features = [list18] + [list36] + [list72] + [x[3]]
        #visual_feature(list18) 
        return features, list18, list36, list72, x[3]

def _hrnet(arch, pretrained_path, **kwargs):
...
def hrnet18(pretrained_path=None, **kwargs):
    r"""HRNet-18 model
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    return _hrnet('hrnet18', pretrained_path, **kwargs)
