...
from .hr_net import hrnet18


@MONO.register_module
class mono_hrnet(nn.Module):
    def __init__(self, options):
        super(mono_hrnet, self).__init__()
        self.opt = options
        self.DepthEncoder = hrnet18(self.opt.depth_pretrained_path)
        self.DepthDecoder = DepthDecoder([ 64, 18, 36, 72, 144 ])
    ...
    def forward(self, inputs):
        outputs = self.DepthDecoder(self.DepthEncoder(inputs["color_aug_zoom", 0, 0]))
        if self.training:
    ...
    # for focus scale sensitive feature
    def disp_to_depth_zoom(self, disp, min_depth, max_depth, ratio):
    ...
    def generate_images_pred(self, inputs, outputs, scale):
        ...
        # _, depth = self.disp_to_depth_zoom(disp, self.opt.min_depth, self.opt.max_depth, inputs[("zoom_scale")])
