    ...
    def after_train_epoch(self, runner):
        ...
        for idx in range(self.dataset.__len__()):
            ...
            if idx == 0 or idx % 20 == 0:
                path = os.path.join("./pred_and_gt/", "{}".format(idx))
                os.makedirs(path, exist_ok=True)
                disp_path = os.path.join("./pred_and_gt/", "{}".format(idx), "disp_{}.png".format(idx))
                diisp = disp.cpu()[0,0].numpy()
                diisp = cv2.resize(diisp, (gt_width, gt_height))
                diisp = 255.0 * (diisp - diisp.min()) / (diisp.max() - diisp.min())
                cv2.imwrite(disp_path, diisp.astype('uint8'))

                # gt_path = os.path.join("./pred_and_gt/", "{}".format(idx), "gt_{}.png".format(idx))
                # gt = data['gt_depth'].cpu()[0].numpy()
                # gt = 255.0 * (gt - gt.min()) / (gt.max() - gt.min())
                # cv2.imwrite(gt_path, gt.astype('uint8'))

                color = 255 * data['color', 0, 0].permute(0,2,3,1).cpu()[0].numpy()
                color_BGR = cv2.cvtColor(color, cv2.COLOR_RGB2BGR)
                color_BGR = np.clip(color_BGR, 0, 255)
                color_path = os.path.join("./pred_and_gt/", "{}".format(idx), "color_{}.png".format(idx))
                cv2.imwrite(color_path, color_BGR.astype('uint8'))
