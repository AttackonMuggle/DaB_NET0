...
model = dict(
    name = 'mono_hrnet',# select a model by name
    ...
    height = HEIGHT,
    width = WIDTH,
    scales = [0],# output different scales of depth maps
    min_depth = 0.1, # minimum of predicted depth value
    max_depth = 100.0, # maximum of predicted depth value
    depth_pretrained_path = './pre_trained/hrnetv2_w18_imagenet_pretrained.pth',# pretrained weights for resnet
...
# resume_from = './output_r2d2/epoch_10.pth'#directly start training from provide weights
resume_from = None
finetune = None
total_epochs = 40
imgs_per_gpu = IMGS_PER_GPU
