...
def evaluate(MODEL_PATH, CFG_PATH, GT_PATH):
    ...
    with torch.no_grad():
        for batch_idx, inputs in enumerate(dataloader):
            ...
            if zoom_post_process:
                disp = zoom_pp(inputs, disp, depth_encoder, depth_decoder)
                disp = zoom_pp_twice(inputs, disp, depth_encoder, depth_decoder, scale=0.75)
    ...
    for i in range(pred_disps.shape[0]):
        ...
        color = cv2.resize(color, (gt_width, gt_height))
...
def save_disp(pred_disp, color, save_path, iter_img):
    ...
    # save disp color
    disp = pred_disp
    vmax = 6.0  # vmax = np.percentile(disp, 95)
...
def zoom_pp(inputs, disp, encoder, decoder, disp_grid=None, scale=0.5):
    _, _, H, W = disp.shape
    h = H * scale
    w = W * scale
    disp_dic = decoder(encoder(inputs[("color_zoom_pp", 0, 0)]))
    disp_zoom = F.interpolate(disp_dic[("disp", 0, 0)], [int(h), int(w)], mode='bilinear', align_corners=False)
    ...
    # do disp fuse
    # percentile 95
    # norm = disp_crop / (np.percentile(disp_crop.detach().cpu().numpy(), 95) + 1e-6)
    # norm[norm > 1] = 1
    # disp_norm = (1 - norm) * disp_crop + norm * disp_zoom
    # disp[:, :, h_start:h_end, w_start:w_end] = disp_norm

    # replace
    # disp[:, :, h_start:h_end, w_start:w_end] = disp_zoom
...
def zoom_pp_twice(inputs, disp, encoder, decoder, disp_grid=None, scale=0.75):
    _, _, H, W = disp.shape
    h = H * scale
    w = W * scale
    disp_dic = decoder(encoder(inputs[("color_zoom_pp_twice", 0, 0)]))
    disp_zoom = F.interpolate(disp_dic[("disp", 0, 0)], [int(h), int(w)], mode='bilinear', align_corners=False)
