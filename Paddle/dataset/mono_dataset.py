...
class MonoDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.SSF_detacher = SSF_detacher
        self.test = test
    ...
    def preprocess(self, inputs, color_aug):
        ...
        for k in list(inputs):
            if "color" in k:
                n, im, i = k
                # print(inputs[(n, im, - 1)]) PIL
                # inputs[(n, im, 0)] = self.resize(inputs[(n, im, - 1)].resize((self.width, self.height), Image.ANTIALIAS))
                inputs[(n, im, 0)] = inputs[(n, im, - 1)].resize((self.width, self.height), Image.ANTIALIAS)
                # print(inputs[(n, im, 0)]) PIL
    ...
    def __getitem__(self, index):
        ...
        inputs[("K")] = paddle.to_tensor(K)
        inputs[("inv_K")] = paddle.to_tensor(inv_K)
